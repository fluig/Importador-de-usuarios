package br.com.bravatec.webdesk.util;

import java.util.List;

public class GetColleagueResult {

	private String active;
    private String adminUser;
    private String colleagueId;
    private String colleagueName;
    private String companyId;
    private String defaultLanguage;
    private String dialectId;
    private String ecmVersion;
    private String emailHtml;
    private String getUser;
    private String login;
    private String mail;
    private String maxPrivateSize;
    private String menuConfig;
    private String passwd;
    private String rowId;
    private String usedSpace;
	private List<Item> getColleagueResult;

	public List<Item> getColleagueResult() {
		return getColleagueResult;
	}

	public void setGetColleagueResult(List<Item> getColleagueResult) {
		this.getColleagueResult = getColleagueResult;
	}
    
	public String getActive() {
		return active;
	}
	public void setActive(String active) {
		System.out.println("setActive");
		this.active = active;
	}
	public String getAdminUser() {
		return adminUser;
	}
	public void setAdminUser(String adminUser) {
		this.adminUser = adminUser;
	}
	public String getColleagueId() {
		return colleagueId;
	}
	public void setColleagueId(String colleagueId) {
		this.colleagueId = colleagueId;
	}
	public String getColleagueName() {
		return colleagueName;
	}
	public void setColleagueName(String colleagueName) {
		this.colleagueName = colleagueName;
	}
	public String getCompanyId() {
		return companyId;
	}
	public void setCompanyId(String companyId) {
		this.companyId = companyId;
	}
	public String getDefaultLanguage() {
		return defaultLanguage;
	}
	public void setDefaultLanguage(String defaultLanguage) {
		this.defaultLanguage = defaultLanguage;
	}
	public String getDialectId() {
		return dialectId;
	}
	public void setDialectId(String dialectId) {
		this.dialectId = dialectId;
	}
	public String getEcmVersion() {
		return ecmVersion;
	}
	public void setEcmVersion(String ecmVersion) {
		this.ecmVersion = ecmVersion;
	}
	public String getEmailHtml() {
		return emailHtml;
	}
	public void setEmailHtml(String emailHtml) {
		this.emailHtml = emailHtml;
	}
	public String getGetUser() {
		return getUser;
	}
	public void setGetUser(String getUser) {
		this.getUser = getUser;
	}
	public String getLogin() {
		return login;
	}
	public void setLogin(String login) {
		this.login = login;
	}
	public String getMail() {
		return mail;
	}
	public void setMail(String mail) {
		this.mail = mail;
	}
	public String getMaxPrivateSize() {
		return maxPrivateSize;
	}
	public void setMaxPrivateSize(String maxPrivateSize) {
		this.maxPrivateSize = maxPrivateSize;
	}
	public String getMenuConfig() {
		return menuConfig;
	}
	public void setMenuConfig(String menuConfig) {
		this.menuConfig = menuConfig;
	}
	public String getPasswd() {
		return passwd;
	}
	public void setPasswd(String passwd) {
		this.passwd = passwd;
	}
	public String getRowId() {
		return rowId;
	}
	public void setRowId(String rowId) {
		this.rowId = rowId;
	}
	public String getUsedSpace() {
		return usedSpace;
	}
	public void setUsedSpace(String usedSpace) {
		this.usedSpace = usedSpace;
	}


}
